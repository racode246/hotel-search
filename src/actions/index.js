...
export const setGeocodeFetched = (address, location) => dispatch => dispatch({ type: 'GEOCODE_FETCHED', address, location });
export const setPlace = place => dispatch => dispatch({ type: 'CHANGE_PLACE', place });
export const setErrorMessage = message => dispatch => dispatch({ type: 'CHANGE_ERROR_MASSAGE', message });
...
export const startSearch = () => (dispatch, getState) => {
  geocode(getState().place)
    .then(({ status, address, location }) => {
      switch (status) {
        case 'OK': {
          dispatch(setGeocodeFetched(address, location));
